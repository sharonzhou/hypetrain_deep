...
df_nvidia['realism_score_binarized'] = df_nvidia['realism_score'] >= 1.0
df_dense['labels_binarized'] = df_dense['labels'] > 0.5
...
for model in models:
    ...
    metrics[model]['auprc'] = skm.average_precision_score(df.loc[df['model']==model, 'labels_binarized'], df.loc[df['model']==model, 'realism_score_binarized'])
    metrics[model]['accuracy'] = skm.accuracy_score(df.loc[df['model']==model, 'labels_binarized'], df.loc[df['model']==model, 'realism_score_binarized'])
print(metrics)
