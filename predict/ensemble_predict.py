...
class EnsemblePredictor(object):
    ...
    def __init__(self, config_path, model_args, data_args,
        ...
        raise NotImplementedError("Need to adapt ensemble for wind.")
    ...
    def predict(self, cache=False):
        ...
        if not is_cached:
                    ...
                    if cache and (predictions_path.exists() and
                        ...
                        if cache:
                            self.logger.log("Writing predictions to " +
                                            f"{predictions_path}.")
                            probs_df.astype(np.float64).to_csv(predictions_path,
                                                               index=False)
            ...
            if cache:
                ...
                ensemble_probs_df.astype(np.float64).to_csv(self.predictions_path,
                                                            index=False)
                self.logger.log("Writing groundtruth to "
