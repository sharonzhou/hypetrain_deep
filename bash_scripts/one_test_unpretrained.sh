...
do
    ...
    declare -A paths=(["$BPWS"]="$BPWS_PATH")
    
    for j in "${!paths[@]}"
    do
        
        python test.py --ckpt_path "${paths[$j]}" --models_test $B --final_csv
        python test.py --ckpt_path "${paths[$j]}" --models_test $P --final_csv
        python test.py --ckpt_path "${paths[$j]}" --models_test $W --final_csv
        python test.py --ckpt_path "${paths[$j]}" --models_test $S --final_csv
